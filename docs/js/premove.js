...
export function premove(state, key) {
   ...
   const color = piece.color,
      ...
      enemies = new Map(
         [...pieces].filter(([_, p]) => p.color === util.opposite(color)),
      ),
      orig = { key, pos: util.key2pos(key) },
      mobility = (ctx) =>
         mobilityByRole[piece.role](ctx) &&
         state.premovable.additionalPremoveRequirements(ctx),
      partialCtx = {
         orig,
         role: piece.role,
         allPieces: pieces,
   ...
   return util.allPosAndKey
      .filter((dest) => mobility({ ...partialCtx, dest }))
      .map((pk) => pk.key);
}
